...
- MermaidJS code has to be fetched and run by the client. It has a modular architecture to import only the necessary code for the diagram types actually used, but still, it will increase your JS client budget, and it means the rendering perfs depend on the client setup ;
...
1. The plugin requires the installation of the [`mermaid`](https://www.npmjs.com/package/mermaid) package in your Starlight project, alongside its own package `starlight-client-mermaid`. You can install both at once with your preferred package manager:
   ...
   ```sh
   npm install starlight-client-mermaid mermaid
   ```
   ...
   ```sh
   pnpm add starlight-client-mermaid mermaid
   ```
   ...
   ```sh
   yarn add starlight-client-mermaid mermaid
   ```
